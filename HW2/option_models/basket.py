...
def basket_price_norm_analytic(
    ...
    disc_fac = np.exp(-texp * intr)
    forward = spot / disc_fac * div_fac
    spot_eq = forward @ weights[:,None] * disc_fac
    
    cov_m = vol * cor_m * vol[:,None]
    vol_norm = np.sqrt(weights @ cov_m @ weights[:,None])
    
    price = normal_formula(strike, spot_eq, vol_norm, texp, intr=intr, divr=divr, cp_sign=cp_sign)
    result = price[0]
    return result
