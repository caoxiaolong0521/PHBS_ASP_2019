...
class NormalModel:
    ...
    def delta(self, strike, spot, vol, texp, intr=0.0, divr=0.0, cp_sign=1):
        ...
        result = cp_sign * ss.norm.cdf(cp_sign * d)
        return result
    ...
    def vega(self, strike, spot, vol, texp, intr=0.0, divr=0.0, cp_sign=1):
        ...
        result = ss.norm.pdf(d) * np.sqrt(texp)
        return result
    ...
    def gamma(self, strike, spot, vol, texp, intr=0.0, divr=0.0, cp_sign=1):
        ...
        result = ss.norm.pdf(d) / (self.vol*np.sqrt(texp))
        return result
    ...
    def impvol(self, price, strike, spot, texp, cp_sign=1):
        ...
        <-- PUT your implementation here
        '''
        impvol_func = lambda _vol: normal_formula(strike, spot, _vol, \
                                                  texp, self.intr, self.divr,\
                                                  cp_sign) - price
        vol = spot.brentq(impvol_func, 0, spot)
        return vol